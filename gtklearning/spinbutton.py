...
class SpinButtonExample:
	...
	def get_value(self, widget, data, spin, spin2, label):
		if data == 1:
			...
			buf = "%0.*f" % (spin2.get_value_as_int(), spin.get_value())
		label.set_text(buf)
	def __init__(self):
		...
		button = gtk.CheckButton("Numberic only input mode")
		...
		button.connect("clicked", self.get_value, 1, spinner1, spinner2, val_label)
		hbox.pack_start(button, True, True, 5)
		
		button = gtk.Button("Value as float")
		button.connect("clicked", self.get_value, 2, spinner1, spinner2, val_label)
		hbox.pack_start(button, True, True, 5)
